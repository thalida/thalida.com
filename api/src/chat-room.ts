...
export class ChatRoom implements DurableObject {
  ...
  private async moderate(
    ...
    console.log(
      `[moderation] checking message ${message.id}: "${message.text}"`,
    );
    ...
    console.log(
      `[moderation] removing message ${message.id} from ${senderInfo.username}`,
    );
    ...
    senderInfo.warnings++;
    console.log(
      `[moderation] ${senderInfo.username} now has ${senderInfo.warnings}/${MAX_WARNINGS} warnings`,
    );
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      ...
      if (response.status === 429) {
        ...
        const waitMs = retryAfter
          ...
          : 1000 * 2 ** attempt;
        console.warn(
          `[moderation] rate limited (429), retrying in ${waitMs}ms (attempt ${attempt + 1}/${retries})`,
        );
