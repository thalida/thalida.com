...
function ForecastIO() {
  PROXY_SCRIPT = '/proxy.php';
	...
	this.requestData = function(latitude, longitude) {
		request_url = PROXY_SCRIPT + '?url=' + latitude + ',' + longitude + '?units=us';
...
function ForecastIOConditions(raw_data) {
	 ...
	 * @return type
	 */
	this.getSunrise = function(format) {
		if(!format) {
			return raw_data.sunriseTime;
		} else {
			return moment.unix(raw_data.sunriseTime).format(format);
		}
		//return raw_data.sunriseTime;
	}
	/**
	 ...
	 * @return type
	 */
	this.getSunset = function(format) {
		if(!format) {
			return raw_data.sunsetTime;
		} else {
			return moment.unix(raw_data.sunsetTime).format(format);
		}
		//return raw_data.sunsetTime;
	}
	/**
