$(document).ready(function(){
    ...
    $.getJSON('https://twitter.com/statuses/user_timeline.json?screen_name=' + twitter_name + '&count=1&callback=?', function(data)      {
        var tweet = data[0].text;
      	tweet = tweet.replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, function(url) {
            return '<a href="'+url+'" target="_blank">'+url+'</a>';
        }).replace(/@([_a-z0-9]+)/ig, function(reply) {
            return  '<a href="https://twitter.com/'+reply.substring(1)+'" target="_blank">@'+reply.substring(1)+'</a>';
        }).replace(/#([_a-z0-9]+)/ig, function(reply) {
